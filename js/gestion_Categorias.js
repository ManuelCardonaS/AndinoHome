$(document).ready(function() {
    ...
    var categoria = id_Categoria_Seleccionada;
    ...
    $("#fil_Imagen_Categoria").change(function() {

        $("#div_Imagen_Seleccionada").empty();
        for (let i = 0; i < this.files.length; i++) {
            ...
            if (imgtags.indexOf(imagetype) !== -1) {
                ...
                let divImg = $("#div_Imagen_Seleccionada");
                let img = $("<img style='height:auto' class='img_Imagen_Categoria'>");
    ...
    $(document).on("click", "#btn_Crear_Categoria", function() {
        ...
        let nombre_Categoria = $("#txt_Nombre_Categoria").val().trim();
        let descripcion_Categoria = $("#txt_Descripcion").val().trim();

        if (nombre_Categoria == '' || descripcion_Categoria == '') {
            mensaje("Debe ingresar todos los datos de la categoría.", colorError, null);
            return;
        }

        var form_data = new FormData();
        let cantidad_Imagenes = 0;
        let imagen = $('#fil_Imagen_Categoria').prop('files');
        ...
        if (cantidad_Imagenes == 0) {
            mensaje("Debe seleccionar la imagen de la categoría.", colorError, null);
            return;
        }

        form_data.append('nombre_Categoria', nombre_Categoria);
        form_data.append('descripcion_Categoria', descripcion_Categoria);

        $.ajax({
            type: 'POST',
            url: base_url + "index.php/administracion/Categoria/guardar_Categoria",
            processData: false,
            contentType: false,
            dataType: 'json',
            data: form_data,
